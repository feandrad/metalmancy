...
object Materials {
    ...
    val ALLOYS = listOf(PEWTER, BRASS, BRONZE, STEEL, ELECTRUM, INVAR)
    val COPPER_LIKE_METALS = listOf(ZINC, TIN, LEAD, NICKEL)
    val IRON_LIKE_METALS = listOf(ALUMINUM, MANGANESE)
    val GOLD_LIKE_METALS = listOf(SILVER, COBALT, LITHIUM)
    val DIAMOND_LIKE_METALS = listOf(PLATINUM, TITANIUM, MITHRIL, ORICHALCUM, URANIUM)
    val SPECIAL_METALS = listOf(MITHRIL, ORICHALCUM)

    val METALS = COPPER_LIKE_METALS + IRON_LIKE_METALS + GOLD_LIKE_METALS + DIAMOND_LIKE_METALS
    val STONES = listOf(MERCURY)
    val ALL = GEMS + SALTS + STONES + METALS + ALLOYS
}
